        ...
        <div className="flex items-start justify-between gap-2 min-h-[44px]">
          <div className="min-w-0 flex-1">
            <h3 
              ...
              title={unitNumber ? `${displayTitle} #${unitNumber}` : displayTitle}
              data-testid={`text-title-${id}`}
            >
              {displayTitle}
              {unitNumber && (
                <span className="text-muted-foreground font-normal"> #{unitNumber}</span>
              )}
            </h3>
        ...
        {isCompact ? (
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {petFriendly && (
                <PawPrint className="h-3.5 w-3.5 text-foreground" />
              )}
            </div>
          </div>
        ) : (
